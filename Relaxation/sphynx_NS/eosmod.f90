...
!=====================================================!
      ...
      USE parameters
      USE eosmodule
      USE eos_module, only: eos_state, eos_einterp, eos_tinterp

      IMPLICIT NONE
      INTEGER status,i
      DOUBLE PRECISION, DIMENSION(nmax):: ueos

      INTEGER keytemp,keyerr,m,mitermax,itov,o
      ...
      DOUBLE PRECISION, PARAMETER::UZERO=(MASSN-ZE)*MeV/mb-cvel*cvel
      ...
      if (eos.eq.1) then
         ...
         if(l.lt.1000) then
            ...
            do i=1,n1
               ...
               yeg=0.5d0
               yelec(i)=(yel+yeg)*0.5d0
               call eos_tinterp(promro(i),yelec(i),xtemp,c(i),state,status)
               dmutot=state%mue-state%muhat
               do while(abs(dmutot).ge.precision.and.(o.le.50))
                  if(dmutot.le.precision)then
                     yel=yelec(i)
                     yelec(i)=(yelec(i)+yeg)*0.5d0
               ...
               enddo
               p(i)=state%p
               u(i)=state%e-UZERO    !LS Eint is negative (shifted)
	    enddo            
         else 
            do i=1,n1
               o=0
               ...
               do while(abs(dmutot).ge.precision.and.(o.le.50))
                  ...
                  o=o+1
               enddo
               call eos_einterp(promro(i),yelec(i),u(i)+UZERO,c(i),state,status) 
               temp(i)=state%t
               p(i)=state%p
            enddo
         endif
